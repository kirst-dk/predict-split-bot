...
# Задержка перед повторным переставлением ордера (секунды)
# Защита от постоянного cancel/replace при частых изменениях цены
# Ордер ставится СРАЗУ после отмены, но следующее переставление не раньше чем через N сек
REPOSITION_COOLDOWN = int(os.getenv('REPOSITION_COOLDOWN', '5'))

# Отступ от лучшего чужого ASK (в тиках)
# 1 = стоим сразу за ним (2-е место) — больше поинтов, но больше риск филла
# 2 = 3-е место — безопаснее, но меньше поинтов
# 3+ = ещё дальше — макс. безопасность, но меньше всего поинтов
ASK_POSITION_OFFSET = int(os.getenv('ASK_POSITION_OFFSET', '1'))
...
def print_config():
    ...
    print(f"   Cooldown переставления: {REPOSITION_COOLDOWN} сек")
